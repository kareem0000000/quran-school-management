...
<head>
    ...
    <style>
        ...
        .stat-card:hover { transform: translateY(-5px); shadow: 0 10px 15px -3px rgba(0,0,0,0.1); }
...
<body class="text-gray-800 custom-scrollbar">
            ...
            <header class="no-print bg-white shadow-sm sticky top-0 z-10 h-20 flex items-center px-8 justify-between">
                <div class="flex items-center gap-6">
                    ...
                    <div class="relative hidden lg:block">
                        ...
                        <input type="text" id="global-search" oninput="App.handleGlobalSearch(this.value)" placeholder="بحث سريع عن طالب..." class="bg-gray-50 border-none rounded-xl pr-10 pl-4 py-2 text-xs font-bold w-64 focus:ring-2 focus:ring-primary outline-none transition-all">
    ...
    <!-- مودال إضافة اختبار جديد (إضافة جديدة) -->
    ...
    <script>
        ...
        const initialData = {
            settings: { salaryPerStudent: 50, pointReward: 5 }, // إعدادات عامة جديدة
            users: [
                { id: 1, username: 'admin', password: '123', role: 'admin', name: 'عبدالله المدير', idNumber: '123456789', dob: '1990-01-01', circleId: null },
                { id: 2, username: 'teacher1', password: '123', role: 'teacher', name: 'أحمد محمد', idNumber: '987654321', dob: '1995-05-15', circleId: 1 },
                { id: 3, username: 'teacher2', password: '123', role: 'teacher', name: 'سعيد علي', idNumber: '456789123', dob: '1992-10-20', circleId: 2 }
            ],
            circles: [
                { id: 1, name: 'حلقة الفجر', teacher: 'أحمد محمد', studentsCount: 0, attendance: '96%', status: 'نشطة' },
                { id: 2, name: 'حلقة العصر', teacher: 'سعيد علي', studentsCount: 0, attendance: '92%', status: 'نشطة' }
            ],
            students: [
        ...
        const App = {
            ...
            startApp() {
                ...
                this.renderSidebar();
                this.navigate(this.currentUser.role === 'admin' ? 'admin-dashboard' : 'teacher-daily');
            ...
            updateUserInfoUI() {
                ...
                document.getElementById('user-role-badge').textContent = this.currentUser.role === 'admin' ? 'مدير المركز' : 'محفظ الحلقة';
            },
            ...
            renderSidebar() {
                ...
                if (this.currentUser.role === 'admin') {
                    ...
                    const items = [
                        ...
                        { id: 'admin-finance', label: 'المالية والرواتب', icon: 'fa-coins' } // تعديل التسمية
                    ];
                        ...
                        ${this.db.circles.length > 0 ? this.db.circles.map(c => `
                            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm relative overflow-hidden group">
                                ...
                                <div class="flex justify-between">
                                    <h4 class="font-black text-primary mb-1">${c.name}</h4>
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ${this.db.users.map(u => `
                                        <tr class="hover:bg-gray-50 transition-all group">
                                            ...
                                            <td class="p-4"><span class="px-2 py-1 rounded-lg ${u.role === 'admin' ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'} text-[10px] font-black">${u.role === 'admin' ? 'مدير' : 'محفظ'}</span></td>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                        <div class="lg:col-span-2">
                            <div class="bg-white rounded-3xl shadow-sm p-6 border border-gray-100">
                                <h3 class="text-lg font-black text-gray-800 mb-6 flex items-center"><i class="fas fa-chart-line ml-3 text-primary"></i> أداء الحلقات الحالية</h3>
                                <div class="overflow-x-auto">
                                    <table class="w-full text-right">
                                        <thead>
                        ...
                        <div class="bg-white rounded-3xl shadow-sm p-6 border border-gray-100 flex flex-col">
                            <h3 class="text-sm font-black text-gray-400 mb-6 self-start tracking-widest uppercase">توزيع المستويات</h3>
                            ...
                            <table class="w-full text-right">
                                <thead class="bg-gray-50 text-[10px] font-black text-gray-400 uppercase">
                                    <tr>
                                        <th class="p-4">الاسم</th>
                                        <th class="p-4">الحلقة</th>
                                        <th class="p-4">التقدم (الأجزاء)</th>
                                        <th class="p-4">النقاط</th> <!-- إضافة خانة النقاط -->
                                    ...
                                    ${students.length > 0 ? students.map(s => {
                                        ...
                                        return `
                                            <tr class="hover:bg-blue-50/10 transition-all">
                                                <td class="p-4">
                                                    <p class="font-black text-sm text-gray-700">${s.name}</p>
                                                    <p class="text-[9px] text-gray-400">المحفوظ: ${s.juzCount || 0} جزء <span class="points-badge ml-1"><i class="fas fa-star text-[8px]"></i> ${s.points || 0}</span></p>
                                                </td>
                                                <td class="p-4">
                                                ...
                                                <td class="p-4"><input type="text" placeholder="سورة/آية" class="daily-input log-hifz" data-id="${s.id}" value="${log.hifz || ''}"></td>
                                                <td class="p-4"><input type="text" placeholder="من-إلى" class="daily-input log-murajaa" data-id="${s.id}" value="${log.murajaa || ''}"></td>
            ...
            saveDailyLogs() {
                ...
                rows.forEach(row => {
                    ...
                    const logData = {
                        ...
                        hifz: row.querySelector('.log-hifz').value,
                        murajaa: row.querySelector('.log-murajaa').value,
                        exam: row.querySelector('.log-exam').value
                    };
                    
                    const idx = this.db.dailyLogs.findIndex(l => l.studentId === studentId && l.date === activeDate);
                    
                    // منطق منح النقاط التلقائي عند الحضور والتقييم الممتاز
                    if (!idx || idx === -1) {
                    ...
                    if (idx > -1) this.db.dailyLogs[idx] = logData;
                    else this.db.dailyLogs.push(logData);
                });
                this.saveDB();
                            ...
                            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:border-primary transition-all">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 mt-4">
                                        <button onclick="App.viewStudentHistory(${s.id})" class="py-2 bg-gray-50 rounded-xl text-[10px] font-black text-gray-500 hover:bg-primary hover:text-white transition-all">سجل المتابعة</button>
                                        <button onclick="App.printStudentCertificate(${s.id})" class="py-2 bg-orange-50 rounded-xl text-[10px] font-black text-orange-600 hover:bg-orange-500 hover:text-white transition-all">شهادة إنجاز</button>
                                    </div>
                                </div>
                            </div>
                ...
                area.innerHTML = `
                    <div class="bg-blue-600 text-white p-8 rounded-3xl mb-8 flex flex-col md:flex-row justify-between items-center shadow-xl">
                        <div>
                            ...
                            <h2 class="text-4xl font-black">${balance.toLocaleString()} ريال</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-r-4 border-r-green-500">
                            ...
                            <h4 class="text-2xl font-black text-green-600">+ ${totalIncome.toLocaleString()} ريال</h4>
                        </div>
                        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm border-r-4 border-r-red-500">
                            ...
                            <h4 class="text-2xl font-black text-red-600">- ${totalExpense.toLocaleString()} ريال</h4>
                        </div>
                    </div>
                                    ...
                                    <tbody class="divide-y divide-gray-50">
                                        ${(f.transactions || []).length > 0 ? f.transactions.slice().reverse().map(t => `
                                            <tr>
                                                ...
                                                <td class="p-4 font-black text-center">${parseFloat(t.amount).toLocaleString()} ريال</td>
                            ...
                            ${this.db.users.filter(u => u.role === 'teacher').map(teacher => {
                                ...
                                return `
                                    <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                        ...
                                        <p class="text-[10px] mb-2">${studentCount} طالب نشط × ${rate} ريال</p>
                                        <p class="text-lg font-black text-primary">${salary.toLocaleString()} ريال</p>
                                    </div>
                                `;
                            }).join('')}
                        </div>
            ...
            viewStudentHistory(studentId) {
                ...
                area.innerHTML = `
                    <div class="mb-6 flex items-center justify-between no-print">
                        <button onclick="App.navigate('${this.currentUser.role === 'admin' ? 'admin-students' : 'teacher-students'}')" class="bg-white border px-4 py-2 rounded-xl text-xs font-black text-gray-500 hover:text-primary transition-all">
            ...
            renderHonorRoll() {
                const topStudents = [...this.db.students]
                    ...
                    .sort((a, b) => (b.points || 0) - (a.points || 0)) // الترتيب حسب النقاط
                    .slice(0, 3);
                ...
                printWindow.document.write(`
                    <dir dir="rtl" style="font-family: 'Cairo', sans-serif; padding: 40px;">
                        ...
                        <table border="1" style="width: 100%; border-collapse: collapse; text-align: right;">
                            <thead style="background: #1a5fb4; color: white;">
                                <tr><th style="padding: 12px;">التاريخ</th><th>البيان</th><th>الفئة</th><th>النوع</th><th>المبلغ</th></tr>
                            </thead>
                            <tbody>
                                ${f.transactions.map(t => `
                                    <tr>
                                        <td style="padding: 10px;">${new Date(t.date).toLocaleDateString('ar-SA')}</td>
                                        <td>${t.desc}</td>
                                        <td>${t.category || 'عام'}</td>
                                        <td style="color: ${t.type === 'income' ? 'green' : 'red'};">${t.type === 'income' ? 'إيراد' : 'مصروف'}</td>
                                        <td style="font-weight: bold;">${t.amount} ر.س</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
            ...
            renderSystemHealth() {
                ...
                const activeStudents = this.db.students.filter(s => s.status === 'نشط').length;
                const healthPercent = totalStudents > 0 ? Math.round((activeStudents / totalStudents) * 100) : 0;
                
                return `
                    <div class="bg-white p-4 rounded-2xl border border-gray-100 mt-4 no-print">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-[10px] font-black text-gray-400">نسبة نشاط الطلاب</span>
                            <span class="text-[10px] font-black text-primary">${healthPercent}%</span>
                        </div>
                        <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                            <div class="bg-primary h-full transition-all duration-1000" style="width: ${healthPercent}%"></div>
                        </div>
                    </div>
                `;
            },
            ...
            printStudentCertificate(studentId) {
                ...
                win.document.write(`
                    <dir dir="rtl" style="font-family: 'Cairo', sans-serif; text-align: center; border: 15px double #1a5fb4; padding: 50px; margin: 20px;">
                        <h1 style="color: #1a5fb4; font-size: 40px;">شهادة تقدير وإنجاز</h1>
                        <p style="font-size: 20px;">يسر مركز الفرقان لتعليم القرآن الكريم أن يمنح الطالب:</p>
                        <h2 style="font-size: 35px; background: #f0f7ff; padding: 10px; display: inline-block; border-radius: 15px;">${student.name}</h2>
                        <p style="font-size: 20px;">هذه الشهادة لتميزه في حفظ القرآن الكريم، حيث أتم حفظ:</p>
                        <h3 style="font-size: 30px; color: #2ec27e;">(${student.juzCount}) جزءاً</h3>
                        <p style="font-size: 20px;">وحصل على رصيد نقاط تميز: ${student.points || 0}</p>
                        <p style="font-size: 18px; margin-top: 40px;">سائلين المولى عز وجل أن يجعله من أهل القرآن الذين هم أهل الله وخاصته.</p>
                    </dir>
                `);
                win.document.close();
            ...
            handleGlobalSearch(query) {
                ...
                const matches = this.db.students.filter(s => s.name.includes(query)).slice(0, 5);
                
                if (matches.length > 0) {
                    resDiv.innerHTML = matches.map(s => `
                        <div onclick="App.navigate('admin-students'); App.renderAdminStudents('${s.name}'); document.getElementById('search-results').classList.add('hidden');" class="p-3 hover:bg-gray-50 cursor-pointer flex justify-between items-center border-b border-gray-50">
                            <div>
                                <p class="text-xs font-black text-gray-800">${s.name}</p>
                                <p class="text-[9px] text-gray-400">${this.db.circles.find(c => c.id == s.circleId)?.name || 'بدون حلقة'}</p>
                            </div>
                            <i class="fas fa-chevron-left text-gray-300 text-[10px]"></i>
                        </div>
                    `).join('');
            ...
            setupTestForm() {
                ...
                form.onsubmit = (e) => {
                    ...
                    // منح نقاط إضافية عند النجاح في الاختبار
                    const sIdx = this.db.students.findIndex(s => s.id == studentId);
                    if(sIdx > -1) {
